...
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "2" #指定使用的 GPU 编号为“0”
config = tf.ConfigProto()
config.gpu_options.allow_growth = True
session = tf.Session(config=config)

class adapt_wcnn_imbalanced(object):
...
#            focal = tf.pow(tf.subtract(1., tf.clip_by_value(pred, 1e-10, 1.0)), class_weight)
            cost1 = tf.reduce_mean(-tf.reduce_sum(y*tf.log(tf.clip_by_value(pred, 1e-10, 1.0)), reduction_indices=[1]))+loss_w
#            cost1=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y))+loss_w
        ...
        with tf.name_scope("loss2"):
            ...
            optm2 = tf.train.AdamOptimizer(learning_rate = learning_rate).minimize(cost2, var_list = output_vars2)
...
#                print(sess.run(cost_weight))
        ...
        return confusion_matrix, accr_confusion1, f_score_out, g_mean_out, test_accr, train_accr, f_score_out_test, g_mean_out_test
#        return test_accr
...
#    y_data = data['planetary_feature_target']
    ...
    #balanced dataset
    x_train_b, x_test_b, y_train_b, y_test_b = train_test_split(x_data, y_data, test_size=0.3)
...
if __name__ == '__main__':
    ...
    f_score_fin = np.zeros((10,100))
    g_mean_fin = np.zeros((10,100))
    f_score_fin_test = np.zeros((10,100))
    g_mean_fin_test = np.zeros((10,100))
    for i in range(10):
        ...
        model_name = 'adapt_wcnn_imbalanced'
        if model_name == 'adapt_wcnn_imbalanced':
            model = adapt_wcnn_imbalanced(x, y,
                                    is_training,
                                    keep_prob,
                                    input_num1=1920,
                                    kernel_number_1=16,
        ...
        train_parameter = {'training_epochs': 100,
                           ...
                           'display_step': 1,
                           'learning_rate': 0.001}
#        model.train(x, y, x_train_i, y_train_i, x_test_i, y_test_i, train_weight, class_weight, ir_overall, train_parameter)
        
        print('%d experience is beginning... \n'%(i))
        #cost_CNN
        confusion_matrix, accr_confusion1, f_score_out, g_mean_out, test_accr, train_accr, f_score_out_test, g_mean_out_test = model.train(x, y, f, x_train_i, y_train_i, x_test_i, y_test_i, train_weight, class_weight, ir_overall, train_parameter)
        #CNN
#        confusion_matrix, accr_confusion1, f_score_out, g_mean_out, test_accr, train_accr, f_score_out_test, g_mean_out_test = model.train(x, y, x_train_i, y_train_i, x_test_i, y_test_i, train_weight, class_weight, ir_overall, train_parameter)
