...
class cost_imbalanced(object):
    ...
    #    out=tf.nn.softmax(out1)
        ...
        return out, cost_weight, loss_w, layer_fn
    
    def train(self, x, y, f, x_train_i, y_train_i, x_test_i, y_test_i, train_weight, class_weight, ir_overall, train_parameter, cost_parameter):
        ...
        with tf.name_scope("loss1"):
            #focal loss
#            gamma = 9
            focal = tf.pow(tf.subtract(1., tf.clip_by_value(pred, 1e-10, 1.0)), 1/class_weight)
            cost1 = tf.reduce_mean(-tf.reduce_sum(focal*y*y*tf.log(tf.clip_by_value(pred, 1e-10, 1.0)), reduction_indices=[1]))+loss_w
        ...
        with tf.name_scope("loss2"):
            #cost sensitive loss
            if cost_parameter == 'g_mean':
                h = ir_overall*tf.exp(-g_mean)
            elif cost_parameter == 'f':
                h = ir_overall*tf.exp(-f)
            elif cost_parameter == 'g_meam_f':
...
#    y_data = data['planetary_feature_target']
    ...
    data = sio.loadmat('dataset_2.mat')
    x_data = data['f_data0']
    y_data = data['f_label']
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        cost_num = 2
        cost_parameter = ['g_mean', 'f', 'g_mean_f']
        
        print('%d experience is beginning... \n'%(i))
        #cost_CNN
#        confusion_matrix, accr_confusion1, f_score_out, g_mean_out, test_accr, train_accr, f_score_out_test, g_mean_out_test, fn_output = model.train(x, y, f, x_train_i, y_train_i, x_test_i, y_test_i, train_weight, class_weight, ir_overall, train_parameter, cost_parameter[cost_num])
        #CNN
        confusion_matrix, accr_confusion1, f_score_out, g_mean_out, test_accr, train_accr, f_score_out_test, g_mean_out_test, fn_output = model.train(x, y, x_train_i, y_train_i, x_test_i, y_test_i, train_weight, class_weight, ir_overall, train_parameter)
